...
module.exports = {
    ...
    async cadastrarUsuarios(request, response) { //request é o que vem do frontend
        try { //sistema try catch, vai passar pela primeira opção, se der erro em alguma linha ele vai entrar na segunda opção e mostrar a mensagem
            return response.status(200).json({
                sucesso: true,
    ...
    async editarUsuarios(request, response) { //request é o que vem do frontend
        try { //sistema try catch, vai passar pela primeira opção, se der erro em alguma linha ele vai entrar na segunda opção e mostrar a mensagem
            return response.status(200).json({
                sucesso: true,
    ...
    async apagarUsuarios(request, response) { //request é o que vem do frontend
        try { //sistema try catch, vai passar pela primeira opção, se der erro em alguma linha ele vai entrar na segunda opção e mostrar a mensagem
            return response.status(200).json({
                sucesso: true,
